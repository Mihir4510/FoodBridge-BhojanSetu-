...
// //
// // FIXES APPLIED:
// //   ✅ FIX 1: Location is non-blocking — user can submit without GPS
// //             GPS runs in background, submits if available, warns if not
// //   ✅ FIX 2: button disabled only when loading, NOT locationLoading
// //   ✅ FIX 3: token stored in localStorage after registration (if returned)
// //   ✅ FIX 4: location fallback — if GPS denied, user can still proceed
...
// // ── Role config ───────────────────────────────────────────
// const ROLES = [
//   { id: "individual",   label: "Donor",      emoji: "🍴", desc: "I have surplus food to donate",    color: "green"  },
//   { id: "organization", label: "NGO",         emoji: "🏠", desc: "We collect & distribute food",    color: "orange" },
//   { id: "restaurant",   label: "Restaurant",  emoji: "🍽️", desc: "We generate daily food surplus",  color: "blue"   },
// ];
...
//         {[1, 2, 3, 4].map((i) => (
//           <div key={i}
//             className={`flex-1 h-1 rounded-full transition-all duration-300 ${i <= strength.score ? strength.color : "bg-[#E5E7EB]"}`}
//           />
//         ))}
//       </div>
//       <p className={`text-[11px] font-semibold ${
//         strength.score <= 1 ? "text-red-500" : strength.score === 2 ? "text-yellow-500"
//         : strength.score === 3 ? "text-blue-500" : "text-[#2D6A4F]"
//       }`}>
...
//   // ── GPS state — NON-BLOCKING ──────────────────────────
//   // Location is captured in background.
//   // If denied/unavailable — user still CAN submit (coordinates = null)
//   // Backend will use a default location or skip geo-matching for this user
...
//   // ── GPS: run silently in background, never block UI ───
//   useEffect(() => {
...
//       // GPS taking too long — allow form submit without it
...
//         setCoordinates([position.coords.longitude, position.coords.latitude]); // GeoJSON [lng, lat]
...
//     <AuthLayout variant="register">
//       <div className="mb-6">
//         <h1 className="font-playfair text-[30px] font-bold text-[#111827] leading-tight mb-1.5">Create your account</h1>
...
//                 isActive ? `${roleAccent.border} ${roleAccent.bg} ring-2 ${roleAccent.ring}/20` : "border-[#E5E7EB] bg-white hover:border-[#D1D5DB]"
//               }`}>
...
//         {role === "organization" && (
//           <InputField label="NGO Name" name="ngoName" value={form.ngoName} onChange={handleChange}
//             placeholder="e.g. Sahara Foundation" icon="🏠" error={errors.ngoName} required />
//         )}
//         {role === "restaurant" && (
//           <InputField label="Restaurant / Hotel Name" name="restaurantName" value={form.restaurantName}
//             onChange={handleChange} placeholder="e.g. Spice Garden" icon="🍽️" error={errors.restaurantName} required />
//         )}

//         <InputField label="Full Name" name="name" value={form.name} onChange={handleChange}
//           placeholder="Your full name" icon="👤" error={errors.name} required autoComplete="name" />

//         <InputField label="Email address" type="email" name="email" value={form.email} onChange={handleChange}
//           placeholder="you@example.com" icon="📧" error={errors.email} required autoComplete="email" />

//         <div>
//           <InputField label="Password" type="password" name="password" value={form.password} onChange={handleChange}
//             placeholder="Min. 8 characters" icon="🔒" error={errors.password} required autoComplete="new-password" />
//           <PasswordStrengthBar password={form.password} />
//         </div>

//         <InputField label="Confirm Password" type="password" name="confirmPassword" value={form.confirmPassword}
//           onChange={handleChange} placeholder="Re-enter your password" icon="🔒"
//           error={errors.confirmPassword} required autoComplete="new-password" />

//         <div className="grid grid-cols-2 gap-3">
//           <InputField label="Phone" type="tel" name="phone" value={form.phone} onChange={handleChange}
//             placeholder="+91 98765 43210" icon="📱" error={errors.phone} required />
//           <InputField label="City" name="city" value={form.city} onChange={handleChange}
//             placeholder="Mumbai" icon="📍" error={errors.city} required />
//         </div>

//         {role === "organization" && (
//           <InputField label="Registration Number" name="registrationNo" value={form.registrationNo}
//             onChange={handleChange} placeholder="e.g. MH/NGO/2021/12345" icon="📋"
//             error={errors.registrationNo} required />
//         )}
//         {role === "restaurant" && (
//           <InputField label="Restaurant Address" name="address" value={form.address} onChange={handleChange}
//             placeholder="Full address" icon="📍" error={errors.address} required />
//         )}
...
//         {/* ✅ FIX: Location status — informational only, never blocks submit */}
//         <div className={`rounded-xl px-4 py-3 flex items-center gap-2.5 text-[12px] font-semibold ${
//           locationStatus === "granted"     ? "bg-[#D8F3DC] text-[#1A4731]"
//           : locationStatus === "loading"  ? "bg-[#F9FAFB] text-[#6B7280]"
...
//           <span>
//             {locationStatus === "granted"
//               ? "Location captured — NGO matching enabled"
//               : locationStatus === "loading"
//               ? "Detecting your location in background..."
//               : "Location not available — you can still register. NGO matching may be limited."}
//           </span>
//         </div>
...
//         {/* ✅ FIX: disabled only on loading — NOT locationLoading */}
//         <button
//           type="submit"
//           disabled={loading}
...
//         <Link to="/login" className="text-[#E76F1A] font-semibold hover:text-[#d4621a] no-underline">Log in →</Link>
//       </p>
//     </AuthLayout>
//   );
// };
...
const Register = () => {
  ...
  // ── Prevent double-submit with a ref (survives re-renders, no stale closure) ──
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ FIX 1: Hard guard — if a request is already in-flight, do nothing.
    // This prevents "user already exists" on impatient double-clicks.
    ...
    try {
      const payload = {
        ...
        role,
        phone:    form.phone,
        // ✅ FIX 2: Send [0,0] if GPS unavailable so backend never rejects the shape
        location: {
          type:        "Point",
          coordinates: coordinates || [0, 0],
        },
      ...
      // ✅ FIX 3: Store token before navigating
      if (res.data?.token) {
        localStorage.setItem("token", res.data.token);
      }

      // ✅ FIX 4: setSuccess → then navigate. No setTimeout race.
      // setSuccess unmounts the form; navigate in a microtask so React
      // finishes the current render cycle first.
      setSuccess(true);

    } catch (err) {
      // ✅ FIX 5: Surface "already registered" clearly so the user knows
      // their first attempt actually worked and they should just log in.
      ...
      if (
        status === 409 ||
        /already exists|already registered|duplicate|email.*taken/i.test(serverMsg)
      ) {
        userFacingError =
          "This email is already registered. Please log in instead, or use a different email.";
      } else if (err.code === "ECONNABORTED" || err.code === "ERR_NETWORK") {
        // ✅ FIX 6: On timeout/network error, warn the user that their account
        // MAY have been created (server received it but response was lost).
        // Tell them to try logging in before registering again.
        userFacingError =
          "The request timed out. Your account may have been created — please try logging in first before registering again.";
      } else {
        userFacingError = serverMsg || "Registration failed. Please try again.";
      }

      setApiError(userFacingError);
      isSubmitting.current = false; // unlock ONLY on error so user can retry
    } finally {
      setLoading(false);
      // Note: we do NOT reset isSubmitting.current on success —
      // the component will unmount (success screen) so it doesn't matter.
    }
  };

  // ── Redirect once success banner is shown ─────────────
  // useEffect so navigate runs after React commits the success render.
  useEffect(() => {
    if (!success) return;
  ...
  return (
    <AuthLayout variant="register">
      ...
      {apiError && (
        <div className="mb-5 bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-[13px] flex items-start gap-2">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
        ...
        {role === "restaurant" && (
          <InputField label="Restaurant / Hotel Name" name="restaurantName" value={form.restaurantName}
            onChange={handleChange} placeholder="e.g. Spice Garden" icon="🍽️"
            error={errors.restaurantName} required />
        )}
        ...
        <div className="grid grid-cols-2 gap-3">
          <InputField label="Phone" type="tel" name="phone" value={form.phone}
            onChange={handleChange} placeholder="+91 98765 43210" icon="📱"
        ...
        {role === "restaurant" && (
          <InputField label="Restaurant Address" name="address" value={form.address}
            onChange={handleChange} placeholder="Full address" icon="📍"
        ...
        {/* Location status — informational only, never blocks submit */}
        <div className={`rounded-xl px-4 py-3 flex items-center gap-2.5 text-[12px] font-semibold ${
          locationStatus === "granted"  ? "bg-[#D8F3DC] text-[#1A4731]"
          : locationStatus === "loading" ? "bg-[#F9FAFB] text-[#6B7280]"
          : "bg-[#FEF3C7] text-[#92400E]"
      ...
      <p className="text-center text-[14px] text-[#6B7280] mt-6">
        Already have an account?{" "}
        <Link to="/login" className="text-[#E76F1A] font-semibold hover:text-[#d4621a] no-underline">
          Log in →
        </Link>
      </p>
    </AuthLayout>
